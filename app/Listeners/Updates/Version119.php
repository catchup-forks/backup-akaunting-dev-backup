...
class Version119 extends Listener
{
    ...
    public function handle(UpdateFinished $event)
    {
        ...
        foreach ($migrations as $model => $name) {
            ...
            foreach ($items as $item) {
                ...
                if (!empty($path) && Storage::exists($path)) {
                    $media = \App\Models\Common\Media::where('filename', '=',
                        pathinfo(basename($path), PATHINFO_FILENAME))->first();
        ...
        $settings['company_logo'] = \App\Models\Setting\Setting::where('key', '=',
            'general.company_logo')->where('company_id', '<>', '0')->get();
        $settings['invoice_logo'] = \App\Models\Setting\Setting::where('key', '=',
            'general.invoice_logo')->where('company_id', '<>', '0')->get();
