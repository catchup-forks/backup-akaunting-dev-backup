...
@if($gateway['mode'] == 'sandbox')
    <div class="alert alert-danger"><i
                class="fa fa-exclamation-circle"></i> {{ trans('paypalstandard::paypalstandard.test_mode') }}</div>
@endif
...
<form action="{{ $gateway['action'] }}" method="post">
    ...
    <input type="hidden" name="return"
           value="{{ url('customers/invoices/' . $invoice->id . '/paypalstandard/result') }}"/>
    <input type="hidden" name="notify_url"
           value="{{ url('customers/invoices/' . $invoice->id . '/paypalstandard/callback') }}"/>
