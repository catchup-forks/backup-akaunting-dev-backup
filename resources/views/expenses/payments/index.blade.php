...
@section('new_button')
    ...
    <span><a href="{{ url('common/import/expenses/payments') }}" class="btn btn-default btn-sm"><span
                    class="fa fa-download"></span> &nbsp;{{ trans('import.import') }}</a></span>
    @endpermission
    <span><a href="{{ route('payments.export', request()->input()) }}" class="btn btn-default btn-sm"><span
            ...
            <div class="table table-responsive">
                <table class="table table-striped table-hover" id="tbl-payments">
                    ...
                    <tr>
                        ...
                        <th class="col-md-2 text-right amount-space">@sortablelink('amount', trans('general.amount'))
                        </th>
                        <th class="col-md-3 hidden-xs">@sortablelink('vendor.name', trans_choice('general.vendors',
                            1))
                        </th>
                        <th class="col-md-2 hidden-xs">@sortablelink('category.name', trans_choice('general.categories',
                            1))
                        </th>
                    ...
                    @foreach($payments as $item)
                        <tr>
                            <td>
                                <a href="{{ url('expenses/payments/' . $item->id . '/edit') }}">{{ Date::parse($item->paid_at)->format($date_format) }}</a>
                            </td>
                                ...
                                @if ($item->category->id != $transfer_cat_id)
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li>
                                                <a href="{{ url('expenses/payments/' . $item->id . '/edit') }}">{{ trans('general.edit') }}</a>
                                            </li>
                                            @permission('create-expenses-payments')
                                            <li class="divider"></li>
                                            <li>
                                                <a href="{{ url('expenses/payments/' . $item->id . '/duplicate') }}">{{ trans('general.duplicate') }}</a>
                                            </li>
                                            @endpermission
