            ...
            <div class="table table-responsive">
                <table class="table table-striped table-hover" id="tbl-categories">
                    ...
                    @foreach($categories as $item)
                        <tr>
                            <td><a href="{{ url('settings/categories/' . $item->id . '/edit') }}">{{ $item->name }}</a>
                            </td>
                            <td>{{ $types[$item->type] }}</td>
                            <td class="hidden-xs"><i class="fa fa-2x fa-circle" style="color:{{ $item->color }};"></i>
                            </td>
                            <td class="hidden-xs">
                                @if ($item->enabled)
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li>
                                            <a href="{{ url('settings/categories/' . $item->id . '/edit') }}">{{ trans('general.edit') }}</a>
                                        </li>
                                        @if ($item->enabled)
                                            <li>
                                                <a href="{{ route('categories.disable', $item->id) }}">{{ trans('general.disable') }}</a>
                                            </li>
                                        @else
                                            <li>
                                                <a href="{{ route('categories.enable', $item->id) }}">{{ trans('general.enable') }}</a>
                                            </li>
                                        @endif
